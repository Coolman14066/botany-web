...
async function initAuth() {
  if (!supabase) return true; // Fallback if supabase library didn't load
  ...
  supabase.auth.onAuthStateChange((event, session) => {
    if (event === 'SIGNED_IN') {
      checkSessionAndRender(session, true);
    } else if (event === 'SIGNED_OUT') {
      showLoginModal();
    }
  });

  return checkSessionAndRender(session, false);
}

function checkSessionAndRender(session, isNewSignIn) {
  const loginOverlay = document.getElementById('auth-overlay');
  if (session && session.user && session.user.email.toLowerCase().endsWith('@accenture.com')) {
    ...
    if (isNewSignIn) {
      // Play the tree growth animation, THEN hide overlay and load content
      playTreeGrowthAndHide().then(() => {
        loadAppContent();
      });
    } else {
      // Returning user with existing session — skip animation
    ...
    if (session && session.user) {
      ...
      alert('Only @accenture.com emails are allowed to access this portal.');
    }
    showLoginModal();
...
function playTreeGrowthAndHide() {
  return new Promise((resolve) => {
    const scene = document.getElementById('auth-plant-scene');
    const container = document.getElementById('auth-container');
    const overlay = document.getElementById('auth-overlay');

    // Fade out the login form card
    if (container) {
    ...
    setTimeout(() => {
      if (scene) {
        scene.classList.add('growing');
      }
    }, 400);

    // After the full tree animation completes (~3.5s), fade out overlay
    setTimeout(() => {
      if (overlay) {
      ...
      setTimeout(() => {
        if (overlay) {
          ...
          overlay.style.opacity = '1'; // Reset for potential logout/re-login
        }
...
window.handleLogin = async function (e) {
  ...
  if (!email.toLowerCase().endsWith('@accenture.com')) {
    alert('Please use an @accenture.com email address.');
    emailInput.focus();
    return;
  }
  ...
  loginBtn.textContent = 'Sending Magic Link...';
